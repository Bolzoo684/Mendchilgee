...
const CreateGreeting: React.FC = () => {
  ...
  const [presets, setPresets] = useState(() => loadPresetGreetings());
  ...
  const handleEditPreset = (greetingId: string) => {
    const greeting = presets.find((g: any) => g.id === greetingId);
    if (greeting) {
  ...
  const handleSaveEdit = () => {
    if (editingPreset) {
      const updatedPresets = presets.map((g: any) =>
          ...
          <div className="bg-white rounded-2xl p-8 max-w-2xl mx-4 shadow-2xl animate-scale-in">
            ...
            <div className="bg-slate-50 rounded-xl p-6">
              <p className="text-slate-700 leading-relaxed whitespace-pre-line">
                {presets.find(g => g.id === previewMode)?.message}
              </p>
            </div>
