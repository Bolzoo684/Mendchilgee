...
const QRScanner: React.FC<QRScannerProps> = ({ onScanResult }) => {
    ...
    const initializeScanner = async () => {
      try {
        ...
        if (devices.length > 0) {
          ...
          setDeviceId(backCamera?.deviceId || devices[0].deviceId);
        } else {
          setHasCamera(false);
  ...
  const startScanning = async () => {
    if (!qrReader || !videoRef.current) return;

    try {
      setIsScanning(true);
      setCameraError(null);
      
      await qrReader.decodeOnceFromVideoDevice(deviceId, videoRef.current)
        .then(result => {
          handleScanResult(result.getText());
        })
        .catch(error => {
          if (error instanceof NotFoundException) {
            // QR код олдоогүй - энэ нь энгийн алдаа, үргэлжлүүлэх
            if (isScanning) {
              setTimeout(() => startScanning(), 100);
            }
          } else {
            console.error('Скан хийхэд алдаа:', error);
            setCameraError('🔍 QR код уншихад алдаа гарлаа');
            setIsScanning(false);
          }
        });
    } catch (error) {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await qrReader.decodeFromInputVideoDevice(undefined, file);
      handleScanResult(result.getText());
    } catch (error) {
